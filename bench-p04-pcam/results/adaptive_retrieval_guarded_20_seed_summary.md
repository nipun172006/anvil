...
Guarded adaptive retrieval passes the 20-seed gate. Compared with stable 20-seed (`+0.048` mean delta, `+0.005` min delta, `1.02x` mean spread, `1.01x` min spread, `0` spread failures, `66.98 / 90`), this config improves retrieval to `+0.055` mean delta and `+0.011` min delta while keeping `0` negative-delta seeds and `0` spread failures. Recommend adopting this guarded retrieval policy if final defaults are allowed to change.
